...
public class Musicbotton : MonoBehaviour {
	...
	public static float MusicVo;            //音樂音量
	int musicing = 1;                       //音樂記錄狀態  
	public Scrollbar Scroll;               //音量卷軸
	...
	void Start () {
		...
		MusicVo = PlayerPrefs.GetFloat("MusicVolume");
		Scroll.value = music.volume;
	...
	public void MusicVolume(float newVolume){
		...
		MusicVo = newVolume;
		Scroll.value = music.volume;
		PlayerPrefs.SetFloat("MusicVolume", MusicVo );
		PlayerPrefs.Save(); //加上這步驟後，存檔就完成了 
	}
