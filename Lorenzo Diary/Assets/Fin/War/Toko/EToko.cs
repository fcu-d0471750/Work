...
public class EToko : MonoBehaviour {

	private Animator animator;
	private bool Run_Punch=false;    //是否遇到過敵人 true:遇到 false:沒遇到
	private float Live = 28.0f;       //生命值
	float Run_PunchT;
	float maxtime;
	...
	void Start () {
		...
		maxtime = 0.3f;
		Live = 28.0f * WarSelect.WarLevel;   //依照關卡難度提升生命值
	...
	void Update () {
		...
		LiveBar.transform.localScale = new Vector3(Live/(28.0f * WarSelect.WarLevel)/10.0f, 0.1f, 0);
		...
		if(LiveAction){
			if ((Live / (28.0f * WarSelect.WarLevel)) < 0.7f)        //如果現在血量低於 70% ，則現在血量 = 現在血量+最大血量*30%
				Live = Live + (28.0f * WarSelect.WarLevel * 0.3f);
			else
				Live = (28.0f * WarSelect.WarLevel);                 //如果現在血量高於 70% ，則現在血量 = 最大血量
			LiveAction = false;
		}
		...
		if (BombAction) {
			if (Live > ((28.0f * WarSelect.WarLevel) * 0.2f))         //只要現在血量大於現在血量20% 則現在血量 = 現在血量-最大血量*20%
				Live = Live - ((28.0f * WarSelect.WarLevel) * 0.2f);
			else {                                                   //現在血量 = 0 播放死亡動畫
				Live = 0.0f;
				Run_Punch  = true;
				animator.SetBool("Run_Punch", Run_Punch);
