...
public class Archer2 : MonoBehaviour {
	...
	void Start () {
		...
		maxtime = 0.3f;
		Live = 7.0f * (1 + (Main.Apeople + Main.Aweapon + Main.Ahospital + Main.Afood) );    //依照建照等級提升生命值
		LiveAction = false;
	}
	...
	void Update () {
		...
		LiveBar.transform.localScale = new Vector3(Live/(7.0f * (1 + (Main.Apeople + Main.Aweapon + Main.Ahospital + Main.Afood)))/2.0f, 0.5f, 0);
		...
		if(LiveAction){
			if ((Live / (7.0f * (1 + (Main.Apeople + Main.Aweapon + Main.Ahospital + Main.Afood)))) < 0.7f)        //如果現在血量低於 70% ，則現在血量 = 現在血量+最大血量*30%
				Live = Live + (7.0f * ((1 + (Main.Apeople + Main.Aweapon + Main.Ahospital + Main.Afood))) * 0.3f);
			else
				Live = (7.0f * (1 + (Main.Apeople + Main.Aweapon + Main.Ahospital + Main.Afood)));                 //如果現在血量高於 70% ，則現在血量 = 最大血量
		...
		if (BombAction) {
			if (Live > ((7.0f * (1 + (Main.Apeople + Main.Aweapon + Main.Ahospital + Main.Afood))) * 0.2f))         //只要現在血量大於現在血量20% 則現在血量 = 現在血量-最大血量*20%
				Live = Live - ((7.0f * (1 + (Main.Apeople + Main.Aweapon + Main.Ahospital + Main.Afood))) * 0.2f);
			else {                                                   //現在血量 = 0 播放死亡動畫
				Live = 0.0f;
				Run_Punch  = true;
				animator.SetBool("Run_Punch", Run_Punch);
