...
public class Holger2 : MonoBehaviour {
	private Animator animator;
	private bool Run_Punch=false; //是否遇到過敵人 true:遇到 false:沒遇到
	private float Live = 3.0f;    //生命值
	float Run_PunchT;             //沒遇到敵人的時間
	float maxtime;                //最多幾秒沒碰到敵人
	...
	void Start () {
		...
		maxtime = 0.3f;
		Live = 3.0f * (1 + (Main.Apeople + Main.Aweapon + Main.Ahospital + Main.Afood) );    //依照建照等級提升生命值
	...
	void Update () {
		if(!CreatePeople.WarPause){//血條生成
		LiveBar.transform.localScale = new Vector3(Live/(3.0f * (1 + (Main.Apeople + Main.Aweapon + Main.Ahospital + Main.Afood)))/10.0f, 0.1f, 0);

		if(Run_Punch)Punchmusic.Play ();        //有遇到敵人播放打擊音效

		if(Run_Punch){
				Run_PunchT=0.0f;           		      //有遇到敵人
		}
		else{
			Run_PunchT += Time.deltaTime;      //沒遇到敵人的時間增加
		}

		Run_Punch = false;

		if (Run_PunchT >= maxtime) {            //沒有遇到敵人超過一定時間
			animator.SetBool("Run_Punch", Run_Punch); //更改為移動
			gameObject.transform.position += new Vector3(0.01f + ((float)Main.Afood/150.0f) , 0 , 0); //不斷向右移動
			Punchmusic.Pause ();                //沒有遇到敵人超過一定時間暫停打擊音效
		}
		...
		if(LiveAction){
			if ((Live / (3.0f * (1 + (Main.Apeople + Main.Aweapon + Main.Ahospital + Main.Afood)))) < 0.7f)        //如果現在血量低於 70% ，則現在血量 = 現在血量+最大血量*30%
				Live = Live + (3.0f * ((1 + (Main.Apeople + Main.Aweapon + Main.Ahospital + Main.Afood))) * 0.3f);
			else
				Live = (3.0f * (1 + (Main.Apeople + Main.Aweapon + Main.Ahospital + Main.Afood)));                 //如果現在血量高於 70% ，則現在血量 = 最大血量
		...
		if (BombAction) {
			if (Live > ((3.0f * (1 + (Main.Apeople + Main.Aweapon + Main.Ahospital + Main.Afood))) * 0.2f))         //只要現在血量大於現在血量20% 則現在血量 = 現在血量-最大血量*20%
				Live = Live - ((3.0f * (1 + (Main.Apeople + Main.Aweapon + Main.Ahospital + Main.Afood))) * 0.2f);
			else {                                                   //現在血量 = 0 播放死亡動畫
				Live = 0.0f;
				Run_Punch  = true;
