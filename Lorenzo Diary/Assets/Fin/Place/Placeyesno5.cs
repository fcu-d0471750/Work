...
public class Placeyesno5 : MonoBehaviour {
	...
	void Start () {
		...
		//讀檔
		Placetime  =  PlayerPrefs.GetInt("Placetime5"); //讀入成就記錄(以下相同)

		if(Placetime!=120) yes = true;  //如果Placetime!=20表示已經按下確定所以yes = true
	}//Start====================================================================================================================
	...
	void Update () {
		...
		//進度圖
		if(Main.yesorno==1 && Placetime!=120) ScoreImaA.SetActive(true);    //進度圖顯示
		else  ScoreImaA.SetActive(false);                 //進度圖隱藏
		ScoreIma.fillAmount = (120.0f-Placetime)/120;     //進度圖變化

		//記錄*****************************************************************************************
		PlayerPrefs.SetInt("Placetime5", Placetime );
	...
	public void obtainButton()
	{
		...
		Main.yesorno = 0 ;
		Main.FadePlay = true;
		int pass = UnityEngine.Random.Range (1, 11); //成功機率50% 

		//成功
		if (pass >= 6) {
			people = UnityEngine.Random.Range (10, 20);   //產生亂數
			weapon = UnityEngine.Random.Range (0, 100);   //產生亂數
			hospital = UnityEngine.Random.Range (0, 100); //產生亂數
			food = UnityEngine.Random.Range (800, 1200);     //產生亂數
			money = UnityEngine.Random.Range (500, 1000);    //產生亂數
		...
		//失敗
			...
			people = UnityEngine.Random.Range (5, 10);   //產生亂數
			weapon = UnityEngine.Random.Range (0, 50);   //產生亂數
			hospital = UnityEngine.Random.Range (0, 50); //產生亂數
			food = UnityEngine.Random.Range (400, 600);     //產生亂數
			money = UnityEngine.Random.Range (250, 500);    //產生亂數

			if(pass==1)Obtaintext.text = "被加勒比海盜\n劫走大部分貨物";
			else if(pass==2)Obtaintext.text = "亞歷山大·漢密爾頓艦隊\n劫走大部分貨物";
