...
public class Placeyesno2 : MonoBehaviour {
	...
	void Start () {
		...
		//讀檔
		Placetime  =  PlayerPrefs.GetInt("Placetime2"); //讀入成就記錄(以下相同)

		if(Placetime!=60) yes = true;  //如果Placetime!=20表示已經按下確定所以yes = true
	}//Start====================================================================================================================
	...
	void Update () {
		...
		//進度圖
		if(Main.yesorno==1 && Placetime!=60) ScoreImaA.SetActive(true);    //進度圖顯示
		else  ScoreImaA.SetActive(false);                 //進度圖隱藏
		ScoreIma.fillAmount = (60.0f-Placetime)/60;       //進度圖變化

		//記錄*****************************************************************************************
		PlayerPrefs.SetInt("Placetime2", Placetime );
	...
	public void obtainButton()
	{
		ButtonMusic.Play();
		yes = false;
		if(Placebutton2.yesno==1)yesbutton.SetActive(true);
		else yesbutton.SetActive(false);
		obtainbutton.SetActive(false);
		Placetime  = 60;
		Main.yesorno = 0 ;
		Main.FadePlay = true;
		int pass = UnityEngine.Random.Range (1, 11); //成功機率80% 

		//成功
		if (pass >= 3) {
			people = UnityEngine.Random.Range (15, 30);     //產生亂數
			weapon = UnityEngine.Random.Range (800, 1000);  //產生亂數
			hospital = UnityEngine.Random.Range (50, 200);  //產生亂數
			food = UnityEngine.Random.Range (10, 500);      //產生亂數
			money = UnityEngine.Random.Range (30, 800);     //產生亂數
		...
		//失敗
			...
			people = UnityEngine.Random.Range (7, 15);     //產生亂數
			weapon = UnityEngine.Random.Range (400, 500);  //產生亂數
			hospital = UnityEngine.Random.Range (25, 100); //產生亂數
			food = UnityEngine.Random.Range (5, 250);      //產生亂數
			money = UnityEngine.Random.Range (15, 400);    //產生亂數

			if(pass==1) Obtaintext.text = "被威廉.丹彼爾海盜學者\n劫走大部分貨物";
			else if(pass==2) Obtaintext.text = "因為觸礁導致貨物丟失"; 
			else Obtaintext.text = "因為觸礁導致貨物丟失"; 
		}
