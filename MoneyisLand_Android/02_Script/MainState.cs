...
public class MainState : MonoBehaviour{
  ...
  //宣告變數============================================================================
	...
	public static ulong OwnMoney = 2000000;
	//持有金錢暫存
	public static ulong OwnMoneyTemp = 0;
	//持有金錢變化(true:變化 false:未變化)
	public static bool OwnMoneyBool = true;
	//切換頁面秒數
	public static float ChangeSceneSec = 1.0f;
	//切換頁面計算秒數
	public static float ChangeSceneSecTemp = 0.0f;
	//經理(共10位)
	...
	//年 月 日 時 分 秒
	public static int[] Lasttime = new int [7];  
  ...
  //宣告狀態============================================================================
	...
	//切換頁面狀態 true:切換畫面中 fasle:沒切換畫面
	public static bool ChangeSceneBool = false;
...
void Update () {
		...
		if (ChangeSceneBool == true) 
			Load_Animation.Load_Animation_Bool = true;
		//否則不播放載入動畫
		else Load_Animation.Load_Animation_Bool = false;
	...
	public static void Write_OwnMoney(){
		...
		int Mid_Four_NUmber = PlayerPrefs.GetInt("Mid_Four_NUmber" , 200); 
	...
	public static void Write_ALL(){
		...
		if (!MainState.BreadArea.GetManage () && MainState.BreadArea.GetStoreNumber () >= 1 && MainState.BreadArea.GetManageTime () < 100.0f) {
			BreadArea.SetManageTime (BreadArea.GetManageTime() + (float)TimeSub);
		}
		//(音樂人)==============================================
		if (!MainState.MusicArea.GetManage () && MainState.MusicArea.GetStoreNumber () >= 1 && MainState.MusicArea.GetManageTime () < 100.0f) {
			MusicArea.SetManageTime (MusicArea.GetManageTime() + (float)TimeSub/3.0f);
		}
		//(科技新貴)==============================================
