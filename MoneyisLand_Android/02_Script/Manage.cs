...
public class Manage : MonoBehaviour {
  ...
  //Image==========================================================
	...
	void Update () {
		//切換頁面中計算是否結束
		if (MainState.ChangeSceneBool == true) {
			MainState.ChangeSceneSecTemp = MainState.ChangeSceneSecTemp + Time.deltaTime;
			//如果超過切換頁面時間則示為切換完成，啟動Update
			if (MainState.ChangeSceneSecTemp >= MainState.ChangeSceneSec) {
				MainState.ChangeSceneBool = false;
				MainState.ChangeSceneSecTemp = 0.0f;
				MainState.ManageBool = true;
               
			}
		}

		if (MainState.ManageBool == true) {
            
            if (MainState.Use_Bool == true) {
			//設定Text==========================================================
			//區域名稱
	...
	public void WorkPage(){
		...
		//切換畫面中
		MainState.ChangeSceneBool = true;
	}
	...
	void LevelEx_ADD(int Invest_Store_Number){
		...
		case 0:
			...
			if (MainState.BreadStore [Invest_Store_Number].GetLevel() != 10) {
				MainState.BreadStore [Invest_Store_Number].SetLevelEx (MainState.BreadStore [Invest_Store_Number].GetLevelEx () + 5.0f);
				LevelEx_Temp = LevelEx_Temp - 5.0f;
			}
		...
		case 1:
			...
			if (MainState.MusicStore [Invest_Store_Number].GetLevel () != 10) {
				MainState.MusicStore [Invest_Store_Number].SetLevelEx (MainState.MusicStore [Invest_Store_Number].GetLevelEx () + 5.0f);
				LevelEx_Temp = LevelEx_Temp - 5.0f;
			}
			//店鋪經驗足以升級==============
		...
		case 2:
			...
			if (MainState.TehcnologyStore [Invest_Store_Number].GetLevel () != 10) {
				MainState.TehcnologyStore [Invest_Store_Number].SetLevelEx (MainState.TehcnologyStore [Invest_Store_Number].GetLevelEx () + 5.0f);
				LevelEx_Temp = LevelEx_Temp - 5.0f;
			}
		...
		case 3:
			...
			if (MainState.FactoryStore [Invest_Store_Number].GetLevel () != 10) {
				MainState.FactoryStore [Invest_Store_Number].SetLevelEx (MainState.FactoryStore [Invest_Store_Number].GetLevelEx () + 5.0f);
				LevelEx_Temp = LevelEx_Temp - 5.0f;
			}
			//店鋪經驗足以升級==============
		...
		case 4:
			...
			if (MainState.EconomicStore [Invest_Store_Number].GetLevel () != 10) {
				MainState.EconomicStore [Invest_Store_Number].SetLevelEx (MainState.EconomicStore [Invest_Store_Number].GetLevelEx () + 5.0f);
				LevelEx_Temp = LevelEx_Temp - 5.0f;
			}
			//店鋪經驗足以升級==============
